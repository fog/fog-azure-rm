...
class TestStartVirtualMachine < Minitest::Test
  ...
  def test_start_virtual_machine_success
    ...
    @promise.stub :value!, response do
      @virtual_machines.stub :start, @promise do
        assert_equal @service.start_virtual_machine('fog-test-rg', 'fog-test-server'), response
      end
    end
  end
  ...
  def test_start_virtual_machine_failure
    ...
    @promise.stub :value!, response do
      @virtual_machines.stub :start, @promise do
        assert_raises(ArgumentError) { @service.start_virtual_machine('fog-test-rg', 'fog-test-server') }
      end
    end
  end
end
