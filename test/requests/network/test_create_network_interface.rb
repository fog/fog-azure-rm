...
class TestCreateNetworkInterface < Minitest::Test
  ...
  def test_create_network_interface_success
    ...
    @promise.stub :value!, mocked_response do
      @network_interfaces.stub :create_or_update, @promise do
        assert_equal @service.create_or_update_network_interface('fog-test-rg', 'fog-test-network-interface', 'West US', 'fog-test-subnet-id', 'fog-test-ip-address-id', 'fog-test-ip-configuration', 'Dynamic', '10.0.0.8'), expected_response
      end
    end
  end
  ...
  def test_create_network_interface_without_public_ip_success
    ...
    @promise.stub :value!, mocked_response do
      @network_interfaces.stub :create_or_update, @promise do
        assert_equal @service.create_or_update_network_interface('fog-test-rg', 'fog-test-network-interface', 'West US', 'fog-test-subnet-id', nil, 'fog-test-ip-configuration', 'Dynamic', '10.0.0.8'), expected_response
      end
    end
  end
