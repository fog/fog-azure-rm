...
class TestUpdateTrafficManagerProfile < Minitest::Test
  ...
  def test_update_traffic_manager_profile_success
    @profiles.stub :create_or_update, @mocked_response do
      assert_equal @service.create_or_update_traffic_manager_profile('fog-test-rg', 'fog-test-profile', 'Weighted', 'relative-name', 35, 'http', 80, '/monitorpage.aspx'), @mocked_response
    end
  end
  ...
  def test_update_traffic_manager_profile_argument_error_failure
    @profiles.stub :create_or_update, @mocked_response do
      assert_raises ArgumentError do
        @service.create_or_update_traffic_manager_profile('fog-test-rg', 'fog-test-profile', 'Performance', 'fog-test-app', 30)
      end
    end
  end
  ...
  def test_update_traffic_manager_profile_exception_failure
    ...
    @profiles.stub :create_or_update, response do
      assert_raises RuntimeError do
        @service.create_or_update_traffic_manager_profile('fog-test-rg', 'fog-test-profile', 'Performance', 'fog-test-app', 30, 'http', 80, '/monitorpage.aspx')
      end
    end
  end
end
