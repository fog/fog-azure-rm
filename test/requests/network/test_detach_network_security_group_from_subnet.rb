  ...
  def test_detach_network_security_group_from_subnet_success
    ...
    @promise.stub :value!, mocked_response do
      @subnets.stub :create_or_update, @promise do
        assert_equal @service.detach_network_security_group_from_subnet('fog-test-rg', 'fog-test-subnet', 'fog-test-virtual-network', '10.1.0.0/24', 'table-id'), expected_response
      end
    end
  end
